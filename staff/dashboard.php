...
function getTotalSales($conn, $id, $year = null, $month = null)
{
    $query = "SELECT SUM(subtotal) FROM sales WHERE user_id = '$id'";

    if ($year) {
        $query .= " AND YEAR(order_date) = " . intval($year);
    }
    if ($month) {
        $query .= " AND MONTH(order_date) = " . intval($month);
    }
...
function getEarningsToday($conn, $id)
{
    $query = "SELECT SUM(subtotal) FROM sales WHERE user_id = '$id' AND DATE(order_date) = CURDATE()";
    $result = mysqli_query($conn, $query);
    return mysqli_fetch_array($result)[0] ?? 0;
}
...
$total_sales = getTotalSales($conn, $id, $selected_year, $selected_month);
$annual_sales = getTotalSales($conn, $id, $selected_year);
$earnings_today = getEarningsToday($conn, $id);
?>
                ...
                <div class="card-body">
                    <div class="row no-gutters align-items-center">
                        <div class="col mr-2">
                            ...
                            <div class="h5 mb-0 font-weight-bold text-gray-800">₱<?= number_format($annual_sales, 2) ?></div>
                        </div>
                        <div class="col-auto">
